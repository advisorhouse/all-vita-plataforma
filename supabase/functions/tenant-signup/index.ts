...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (role === "partner") {
      // Generate unique referral code (will be regenerated by trigger if null/dup)
      const referralCode = `${tenant.slug?.toUpperCase().replace(/-/g, "").slice(0, 4)}-${user.id.slice(0, 6).toUpperCase()}`;

      // Resolve parent partner from referral code if provided
      let parentPartnerId: string | null =
        metadata?.parent_partner_id || user.user_metadata?.parent_partner_id || null;

      const parentRefCode: string | undefined =
        metadata?.parent_referral_code || user.user_metadata?.parent_referral_code;

      if (!parentPartnerId && parentRefCode) {
        const { data: parentRows, error: parentErr } = await supabaseAdmin
          .from("partners")
          .select("id, user_id")
          .eq("tenant_id", tenant_id)
          .eq("referral_code", parentRefCode.toUpperCase())
          .eq("active", true)
          .limit(1);

        if (parentErr) {
          console.error("Resolve parent referral error:", parentErr);
        } else if (parentRows && parentRows.length > 0) {
          // Anti self-referral
          if (parentRows[0].user_id !== user.id) {
            parentPartnerId = parentRows[0].id;
          }
        }
      }
      ...
      const { error: partnerError } = await supabaseAdmin
        ...
        .insert({
          ...
          level: "bronze",
          active: true,
          metadata: { ...(metadata || {}), parent_referral_code: parentRefCode || null },
        });
